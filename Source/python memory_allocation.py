...
print("DEBUG darkhast:", darkhast)
print("DEBUG hofreh:", hofreh)
...
for i in darkhast:
    ...
    for j in range(len(hofreh)):
        if hofreh[j] >= i:
            print(f"{i} -> allocated in block {hofreh[j]} (index {j})")
            hofreh[j] -= i
            print(f"    remaining in that block: {hofreh[j]}")
            allocated = True
            break
